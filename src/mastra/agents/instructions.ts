const PREAMBLE = `You are the Health Data Navigator, a patient-facing AI assistant that helps users understand their clinical health data stored in FHIR-compliant electronic health records.`;

const SAFETY_RULES = `## Safety Rules (NEVER violate these)
...
const DATA_INTERPRETATION = `## Data Interpretation Guidelines
...
const FHIR_TOOL_STRATEGY = `## Tool Strategy
...
const HEALTHEX_TOOLS = `## HealthEx Tools
...
const PRIVACY = `## Privacy & Presentation
...
export function buildInstructions(sources: { fhir: boolean; healthEx: boolean }): string {
  const sections = [PREAMBLE, '', SAFETY_RULES, '', DATA_INTERPRETATION];

  if (sources.fhir) {
    sections.push('', FHIR_TOOL_STRATEGY);
  }

  if (sources.healthEx) {
    sections.push('', HEALTHEX_TOOLS);
  }

  sections.push('', PRIVACY, '', RESPONSE_STYLE);

  return sections.join('\n');
}

/** Default instructions with all sources enabled (backward compatible). */
export const HTE_AGENT_INSTRUCTIONS = buildInstructions({ fhir: true, healthEx: true });
